...
const firebaseConfig = {
  apiKey: "AIzaSyC31_qId7Lhujt2Juba6ofir_MQlYF1cpo",
  authDomain: "the-ingredient-company.firebaseapp.com",
  projectId: "the-ingredient-company",
  storageBucket: "the-ingredient-company.firebasestorage.app",
  messagingSenderId: "546267353192",
  appId: "1:546267353192:web:7338525a1e91b82065abdd",
...
async function bootAdmin() {
  ...
  await Promise.all([fetchCustomers(), fetchPromos(), fetchProducts(), fetchCategories(), fetchBanners(), loadStoreSettings()]);
  renderDashboard();
}
  ...
  c.innerHTML = `<table>
    ...
    <tbody>${orders.map(o=>`<tr>
      ...
      <td style="display:flex;gap:8px;">
        <button class="btn-icon" onclick="openOrderModal('${o._id}')" title="View Order Details"><i class="fas fa-eye"></i></button>
        <button class="btn-icon" onclick="printOrderBill('${o._id}', event)" title="Print Bill" style="color:#4f46e5;border-color:#e0e7ff;background:#eef2ff;"><i class="fas fa-print"></i></button>
        <button class="btn-icon" onclick="sendWhatsAppEBill('${o._id}', event)" title="Send eBill on WhatsApp" style="color:#25d366;border-color:#bbf7d0;background:#f0fdf4;"><i class="fab fa-whatsapp"></i></button>
        <button class="btn-icon danger" onclick="deleteOrder('${o._id}', event)" title="Delete Order"><i class="fas fa-trash"></i></button>
      </td>
    </tr>`).join('')}</tbody>
  </table>`;
}
...
function openOrderModal(id) {
  ...
  body.innerHTML = `
    ...
    <div style="display:flex; flex-direction:column; gap:16px; border-top:1px solid var(--border); padding-top:15px; margin-top:10px;">
      ...
      <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px; border-top: 1px dashed var(--border); padding-top: 12px;">
        <div style="display:flex; gap:10px; flex-wrap:wrap;">
          <button class="btn-primary" onclick="printOrderBill('${o._id}', event);" style="padding: 10px 16px; border-radius: 8px; background: #4f46e5; color: white; border: none; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 8px;">
            <i class="fas fa-print"></i> Print Bill
          </button>
          <button class="btn-primary" onclick="sendWhatsAppEBill('${o._id}', event);" style="padding: 10px 16px; border-radius: 8px; background: #25D366; color: white; border: none; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 8px;">
            <i class="fab fa-whatsapp"></i> Send WhatsApp eBill
          </button>
        </div>
...
// ===== PRINT ORDER BILL =====
function printOrderBill(id, event) {
  if (event) event.stopPropagation();
  const o = allOrders.find(x => x._id === id);
  if (!o) { toast('Order not found', 'error'); return; }

  let printFrame = document.getElementById('qs-print-frame');
  if (!printFrame) {
    printFrame = document.createElement('iframe');
    printFrame.id = 'qs-print-frame';
  ...
  const itemsHtml = (o.items || []).map(item => `
    <tr>
      <td style="padding:2px 0;text-align:left;">${item.name}</td>
      <td style="padding:2px 0;text-align:center;">${item.quantity}</td>
      <td style="padding:2px 0;text-align:right;">₹${item.price || 0}</td>
      <td style="padding:2px 0;text-align:right;">₹${(item.price * item.quantity) || 0}</td>
    </tr>
  `).join('');
  ...
  const receiptHtml = `
    ...
    <head>
      <meta charset="UTF-8">
      <title>Order Bill #${o.id || o._id}</title>
      <style>
        ...
        * { box-sizing: border-box; }
        ...
        .receipt-header { text-align: center; margin-bottom: 6px; border-bottom: 1px dashed #000; padding-bottom: 4px; }
        .receipt-header h2 { margin: 0 0 2px 0; font-size: 13px; text-transform: uppercase; font-weight: 800; }
        .receipt-header p { margin: 1px 0; font-size: 9px; }
        .info-table { width: 100%; margin-bottom: 6px; font-size: 9px; border-collapse: collapse; }
        .info-table td { padding: 1px 0; vertical-align: top; }
        .items-table { width: 100%; border-collapse: collapse; margin-bottom: 6px; border-top: 1px dashed #000; border-bottom: 1px dashed #000; }
        .items-table th { padding: 3px 0; border-bottom: 1px dashed #000; font-size: 9px; text-transform: uppercase; font-weight: 700; }
        .totals-table { width: 100%; margin-bottom: 6px; font-size: 9px; border-collapse: collapse; }
        .totals-table td { padding: 2px 0; }
        .grand-total { font-weight: 800; font-size: 11px; border-top: 1px solid #000; border-bottom: 1px double #000; }
        .receipt-footer { text-align: center; font-size: 8.5px; border-top: 1px dashed #000; padding-top: 4px; margin-top: 4px; }
      </style>
    </head>
    <body>
      ...
      <table class="info-table">
        <tr><td><strong>Customer:</strong></td><td style="text-align:right;">${o.customerName || 'Customer'}</td></tr>
        <tr><td><strong>Phone:</strong></td><td style="text-align:right;">${o.customerPhone || 'N/A'}</td></tr>
        <tr><td><strong>Address:</strong></td><td style="text-align:right;">${o.address || 'N/A'}</td></tr>
        <tr><td><strong>Payment:</strong></td><td style="text-align:right;">${o.paymentMethod || 'COD'}</td></tr>
        <tr><td><strong>Status:</strong></td><td style="text-align:right;">${o.status || 'Pending'}</td></tr>
      </table>
      ...
      <table class="totals-table">
        <tr><td>Subtotal</td><td style="text-align:right;">₹${o.subtotal || o.total || 0}</td></tr>
        <tr><td>Delivery Fee</td><td style="text-align:right;">₹${o.deliveryFee || 0}</td></tr>
        ${o.discount ? `<tr><td>Discount</td><td style="text-align:right;">-₹${o.discount}</td></tr>` : ''}
        <tr class="grand-total">
          <td style="padding-top:3px;padding-bottom:3px;">TOTAL AMOUNT</td>
          <td style="text-align:right;padding-top:3px;padding-bottom:3px;">₹${o.total || 0}</td>
        </tr>
      </table>
...
// ===== WHATSAPP EBILL =====
function generateEBillText(o) {
  const itemsText = (o.items || []).map((item, idx) => 
...
*Order ID:* #${o.id || o._id}
*Date:* ${o.date || '—'}
*Customer:* ${o.customerName || 'Valued Customer'}
...
Delivery Fee: ₹${o.deliveryFee || 0}${o.discount ? `\nDiscount: -₹${o.discount}` : ''}
*TOTAL AMOUNT:* ₹${o.total || 0} (${o.paymentMethod || 'COD'})
--------------------------------
📍 *Delivery Address:*
${o.address || '—'}
...
function sendWhatsAppEBill(orderId, event) {
  if (event) event.stopPropagation();
  ...
  if (!phone) {
    phone = prompt('Customer phone number is missing. Please enter phone number (with country code):');
    if (!phone) return;
    phone = phone.replace(/\D/g, '');
  }

  const formattedPhone = phone.length === 10 ? '91' + phone : phone;
  const text = encodeURIComponent(generateEBillText(o));
  window.open(`https://wa.me/${formattedPhone}?text=${text}`, '_blank');
  toast('Opening WhatsApp eBill ✓', 'success');
}

function notifyWhatsApp(orderId) {
  sendWhatsAppEBill(orderId);
}
